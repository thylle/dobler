<template>
  <div class="coupon">
    <div class="coupon__header">
        Mulig gevinst: <strong>{{profit}},-</strong>
    ...
    <div class="coupon__content">
        <div class="coupon__odds" v-for="item in selectedOdds">
            <span>
                {{item.hometeam}} <strong>vs</strong> {{item.awayteam}}
            </span>
    ...
    export default {
        ...
        methods: {
            saveCoupon: function(){
                ...
                //Gather alle odds and push to new let.
                for (let i = 0; i < this.selectedOdds.length; i++) {
                    let item = this.selectedOdds[i];
                ...
                console.log("save coupon to user: ", this.user);
            }
        }
    }
</script>
